...
function selectTrainDataVideo(methodPill, scenePill, modePill) {
    ...
    pill = methodPill.getAttribute("data-value");
    let train_data_desc = {
        "GuidedCrossAttn": "Spatial cross-attention plays an essential role in T2V generation, as it serves as the only pathway for the prompt to embed into the latent representations. Therefore, it is crucial to thoroughly investigate methods to steer spatial cross-attention towards our desired outcomes. Rather than using a linear additive mask, we employ a Gaussian function to smoothly construct an additive mask corresponding to the bbox area with an influence coefficient 𝜆, allowing it to guide without negatively disrupting the attention values, thereby preserving the quality of generated content.",
        "OrientedAttnSharing": "In our layer generation framework, the fusion of foreground and background is a critical focus. We aim to avoid a scenario where the foreground layer appears to “float above the background layer without interactions, thereby losing visual concordance. To address this, we introduce an orientation step within the attention-sharing process, guiding foreground pixels inside the bbox to attend more closely to their corresponding background pixels",
        "AttnIsolation": "Attn-Isolation separates foreground and background latents to avoid them from disrupting in temporal transformer.",
        "KeyframeAmp": "In addition, for complex trajectories, simple guidance may not suffice to achieve precise bbox-prompt alignment. To address this, we introduce the concept of key-frames. For example, in a polyline trajectory, the start point, end point, and turning point determine the overall movement of the object and are critical to bbox–prompt alignment. Consequently, we amplify the additive mask of these key-frames by a reinforcement parameter 𝛾 to ensure more effective guidance.",
        "Post-Harmonization": "Our method holds a natural advantage in applying masked harmonization, as our generation process operates in a layer-by-layer fashion. This allows us to easily extract binarized alpha maps to serve as foreground masks. In this case, we adopt INR-Harmonization, and as illustrated, the harmonized goose appears more coherent with the background lake in both tone and composition.",
    }[pill]
    var p_desc = document.getElementById("p-traindata-desc");
    p_desc.innerHTML = train_data_desc;

    var image_active = document.getElementById("traindataImage0");
    image_active.src = "assets/videos/train_data/" + pill + ".png";
}
